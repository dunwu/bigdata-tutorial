...
- [2. 消费者 API](#2-消费者-api)
  - [2.1. 创建消费者](#21-创建消费者)
  - [2.2. 订阅主题](#22-订阅主题)
  - [2.3. 轮询获取消息](#23-轮询获取消息)
  - [2.4. 手动提交偏移量](#24-手动提交偏移量)
  - [2.5. 关闭连接](#25-关闭连接)
- [3. 分区再均衡](#3-分区再均衡)
  - [3.1. 什么是分区再均衡](#31-什么是分区再均衡)
  - [3.2. 何时生分区再均衡](#32-何时生分区再均衡)
  - [3.3. 分区再均衡的过程](#33-分区再均衡的过程)
  - [3.4. 如何判定消费者已经死亡](#34-如何判定消费者已经死亡)
  - [3.5. 查找协调者](#35-查找协调者)
  - [3.6. 分区再均衡的问题](#36-分区再均衡的问题)
  - [3.7. 避免分区再均衡](#37-避免分区再均衡)
- [4. 提交偏移量](#4-提交偏移量)
  - [4.1. 偏移量的用处](#41-偏移量的用处)
  - [4.2. 提交偏移量的旧方案](#42-提交偏移量的旧方案)
  - [4.3. 提交偏移量的新方案](#43-提交偏移量的新方案)
  - [4.4. 自动提交](#44-自动提交)
  - [4.5. 手动提交](#45-手动提交)
- [5. 反序列化器](#5-反序列化器)
- [6. 独立消费者](#6-独立消费者)
...
![](https://raw.githubusercontent.com/dunwu/images/dev/snap/20210412200354.png)

（2）**如果提交的偏移量大于客户端处理的最后一个消息的偏移量，那么处于两个偏移量之间的消息将会丢失**。

![](https://raw.githubusercontent.com/dunwu/images/dev/snap/20210412200405.png)
